...
function run_code_tests {
    ...
    run_test "check files included" "python3 tools/validate_dme.py < broken-dreams.dme"
}
...
function run_byond_tests {
    ...
    run_test "build map unit tests" "scripts/dm.sh -DUNIT_TEST -M$MAP_PATH broken-dreams.dme"
    run_test "check no warnings in build" "grep ', 0 warnings' build_log.txt"
    run_test "run unit tests" "DreamDaemon broken-dreams.dmb -invisible -trusted -core 2>&1 | tee log.txt"
