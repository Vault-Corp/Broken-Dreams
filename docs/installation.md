...
This is a sourcecode-only release, so the next step is to compile the server files.  Open `broken-dreams.dme` by double-clicking it, open the Build menu, and click compile.  This'll take a little while, and if everything's done right you'll get a message like this:

    saving broken-dreams.dmb (DEBUG mode)

    broken-dreams.dmb - 0 errors, 0 warnings
